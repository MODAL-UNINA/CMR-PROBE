#!/usr/bin/env python3
"""Fit frozen common-FPR operating thresholds."""
...
def load_calibration_cache(path: Path) -> dict[str, dict[str, np.ndarray]]:
    path = Path(path)
    if not path.exists():
        raise FileNotFoundError(f'Cache of calibration not found: {path}')
    with open(path, 'rb') as f:
        cached = pickle.load(f)
    if 'msts' not in cached or 'mmstar' not in cached:
        raise KeyError(f'Unrecognized cache schema. Available keys: {list(cached.keys())}')
    if len(cached['msts']) != 3 or len(cached['mmstar']) != 3:
        raise ValueError('The cache must contain tuples (S_T, S_V, S_C).')
    st_u, sv_u, sc_u = cached['msts']
    st_s, sv_s, sc_s = cached['mmstar']
    sc_representation = cached.get('sc_representation')
    if sc_representation not in ('mean', 'aligned'):
        raise ValueError('Calibration cache is missing valid sc_representation metadata.')
    out = {'sc_representation': sc_representation, 'unsafe': {'T': _as_1d(st_u, 'MSTS S_T'), 'V': _as_1d(sv_u, 'MSTS S_V'), 'C': _as_1d(sc_u, 'MSTS S_C')}, 'safe': {'T': _as_1d(st_s, 'MMStar S_T'), 'V': _as_1d(sv_s, 'MMStar S_V'), 'C': _as_1d(sc_s, 'MMStar S_C')}}
...
def stratified_fit_operating_split(n_unsafe: int, n_safe: int, fit_fraction: float, seed: int) -> dict[str, np.ndarray]:
    if not 0.1 <= fit_fraction <= 0.9:
        raise ValueError('fit_fraction must be between 0.1 and 0.9')
...
def stack_features(scores: dict[str, np.ndarray], idx: np.ndarray) -> np.ndarray:
    return np.column_stack([scores['T'][idx], scores['V'][idx], scores['C'][idx]])

def fit_learned_fusion(unsafe: dict[str, np.ndarray], safe: dict[str, np.ndarray], split: dict[str, np.ndarray], seed: int) -> Pipeline:
...
def build_method_scores(scores: dict[str, np.ndarray], idx: np.ndarray, learned_model: Pipeline) -> dict[str, np.ndarray]:
    st = scores['T'][idx]
    sv = scores['V'][idx]
    sc = scores['C'][idx]
...
def threshold_at_fpr(scores_safe: np.ndarray, target_fpr: float) -> tuple[float, float, int]:
...
def operating_metrics(scores_unsafe: np.ndarray, scores_safe: np.ndarray, tau: float) -> dict:
...
def calibrate_one_method(name: str, score_u: np.ndarray, score_s: np.ndarray, targets: tuple[float, ...]) -> dict:
    result = {'score_definition': {'S_C': 'S_C', 'S_T+S_V': 'max(S_T, S_V)', 'GEOMETRIC': 'max(S_T, S_V, S_C)', 'LEARNED': 'LogisticRegression(S_T, S_V, S_C)'}[name], 'n_unsafe_operating_cal': int(len(score_u)), 'n_safe_operating_cal': int(len(score_s)), 'operating_points': {}}
    for target in targets:
        tau, empirical_fpr, n_fp = threshold_at_fpr(score_s, target)
        m = operating_metrics(score_u, score_s, tau)
        key = f'fpr_{int(round(target * 100)):02d}'
        result['operating_points'][key] = {'target_fpr': target, 'tau': tau, 'empirical_fpr_from_safe_cal': empirical_fpr, 'n_fp_safe_cal': n_fp, **m}
    return result

def run(args):
    delta_dir = Path(args.delta)
    delta_dir.mkdir(parents=True, exist_ok=True)
    cal_cache = Path(args.cal_cache) if args.cal_cache else 'probes/rebuilt_v13/delta/scores_cal_seed42_msts100_mmstar400.pkl'
    print('\n' + '=' * 78)
    print('COMMON-FPR CALIBRATION — CALIBRATION ONLY')
    print('=' * 78)
    ...
    data = load_calibration_cache(cal_cache)
    sc_representation = data['sc_representation']
    unsafe, safe = (data['unsafe'], data['safe'])
    n_u, n_s = (len(unsafe['C']), len(safe['C']))
    ...
    if len(split['safe_operating']) < 100:
        print('[WARN] fewer than 100 benign samples in operating_cal: the 1% FPR point has coarse empirical resolution.')
    learned = fit_learned_fusion(unsafe, safe, split, args.split_seed)
    model_path = delta_dir / 'learned_fusion_common_fpr.pkl'
    ...
    methods = {}
    print('\n' + '-' * 78)
    print('OPERATING POINTS (selected only on operating_cal)')
    print('-' * 78)
    for name in METHODS:
    ...
    config = {'protocol': {'name': 'common_fpr_calibration', 'sc_representation': sc_representation, 'test_used': False, 'primary_target_fpr': PRIMARY_FPR, 'secondary_target_fpr': SECONDARY_FPR, 'threshold_rule': 'smallest empirical threshold with safe-cal FPR <= target; unsafe iff score >= tau', 'learned_fusion_fit': 'separate fusion_fit subset of calibration only', 'threshold_calibration': 'shared operating_cal subset for all methods', 'quantile_matching': False, 'youden_for_final_operating_point': False}, 'source': {'calibration_cache': str(cal_cache), 'unsafe_dataset': 'MSTS_cal', 'safe_dataset': 'MMStar_cal', 'n_unsafe_total': n_u, 'n_safe_total': n_s, 'source_seed': args.source_seed}, 'split': {'split_seed': args.split_seed, 'fusion_fit_fraction': args.fusion_fit_fraction, 'unsafe_fit_positions': split['unsafe_fit'].tolist(), 'unsafe_operating_positions': split['unsafe_operating'].tolist(), 'safe_fit_positions': split['safe_fit'].tolist(), 'safe_operating_positions': split['safe_operating'].tolist()}, 'methods': methods, 'learned_model_path': str(model_path)}
    cfg_path = delta_dir / 'common_fpr_config.json'
    with open(cfg_path, 'w', encoding='utf-8') as f:
        json.dump(config, f, indent=2, ensure_ascii=False)
    score_cache_path = delta_dir / 'common_fpr_calibration_scores.pkl'
    with open(score_cache_path, 'wb') as f:
        pickle.dump({'sc_representation': sc_representation, 'scores_unsafe_operating': scores_u, 'scores_safe_operating': scores_s, 'split': split, 'source_calibration_cache': str(cal_cache)}, f)
    print('\n' + '=' * 78)
    print('SAVED')
    print('=' * 78)
    ...
    print('\nNo test set was loaded or used.')
if __name__ == '__main__':
    import sys
    CONFIG = {'delta': 'probes/rebuilt_v13/delta', 'cal_cache': None, 'n_cal_msts': 200, 'n_cal_mmstar': 400, 'source_seed': 42, 'split_seed': 2026, 'fusion_fit_fraction': 0.5}
    ...
    if not in_jupyter:
        ...
        parser.add_argument('--n_cal_msts', type=int, default=CONFIG['n_cal_msts'])
